...
module.exports.run = (client, message) => {
		...
		const lieu     = [`à Londres`, `à Stockholm`, `en nouvelle Zélande`, `dans son salon`, `dans la cuisine du voisin`, `sur l'Empire Du Poulet`, `dehors`, `dans une église`, `dans la cave`, `dans un laboratoire`, `dans une maison close`, `dans l'espace`, `dans la chambre de Zheo`, `dans un cimetière`, `à un mariage`, `dans la cathédrale Dieu Poulet`, `dans un centre commercial`, `dans les toilettes`, `dans un cinéma`, `au Super U du coin`, `à Auschwitz`]; //lieu
        ...
        const personne_ran = randomInt(0, personne.length);
        const action_ran = randomInt(0, action.length);
        const objet_ran = randomInt(0, objet.length);
        const conjCoord_ran = 0
        const objet2_ran = randomInt(0, objet2.length);
        const lieu_ran = randomInt(0, lieu.length);
...
module.exports.help = {
    ...
    desription: "Génère une phrase 'drôle'.",
    usage: "",
    cooldown: 1,
    permissions: false,
    args: false
