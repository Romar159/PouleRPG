module.exports.run = (client, message) => {
    client.writeLog(`Commande ${this.help.name} executée par ${message.author.tag} (${message.author.id})`);    
        ...
        client.writeLog(`Commande ${this.help.name} : ${message.author.tag} (${message.author.id}) - PHRASE=${personne[personne_ran]} (id=${personne_ran}) ${action[action_ran]} (id=${action_ran}) ${objet[objet_ran]} (id=${objet_ran}) ${conjCoord[conjCoord_ran]} (id=${conjCoord_ran}) ${objet2[objet2_ran]} (id=${objet2_ran}) ${lieu[lieu_ran]} (id=${lieu_ran})`);    
    }
