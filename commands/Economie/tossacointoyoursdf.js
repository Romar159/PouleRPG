...
module.exports.run = async (client, message, args, settings, dbUser) => {
    ...
    if(lastDaily !== null && dailyCD - (Date.now() - lastDaily) > 0) { //cooldown pas encore passé.
        ...
        message.reply(`Il n'y a plus aucun SDF dans le coin, attendez encore ${Math.floor(cdTime / (1000*60*60) % 24)} Heures, ${Math.floor(cdTime / (1000*60) % 60)} Minutes et ${Math.floor(cdTime / (1000) % 60)} secondes, il devrait y en avoir de nouveau.`);
    } else { // Si le cooldown est passé.

        if(dbUser.or <= 0) return message.channel.send("Vous êtes vous même un SDF, vous ne pouvez pas donner d'argent.");
...
module.exports.help = {
    ...
    category: "economie",
    desription: "Envoie un de votre or à un utilisateur aléatoire, vous permettant de gagner de l'XP, et avec de la chance un point d'amitié.",
    usage: "",
    cooldown: 1,
    permissions: false,
    args: false
