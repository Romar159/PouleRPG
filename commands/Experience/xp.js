    ...
    if(message.mentions.users.first()) {
        ...
        try {
            if(usr1.or < 0) { // Si l'utilisateur n'existe pas dans la bdd on le crée. (en passant dans le catch).
                ...
                const needxp_usr = Math.round((1000 * parseInt(userToUpdate.level)) / Math.sqrt(parseInt(userToUpdate.level)) * 1.5);
        ...
        const needxp_dbuser = Math.round((1000 * parseInt(userToUpdate.level)) / Math.sqrt(parseInt(userToUpdate.level)) * 1.5);
